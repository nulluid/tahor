...
class PrivateBackupTests(unittest.TestCase):
    def setUp(self):
        ...
        self.sources['business_filing.json'].write_text('{"version":1,"businesses":[]}')
    ...
    def test_roundtrip_private_rules_database_credentials_and_permissions(self):
        ...
        self.sources['business_filing.json'].write_text('{}')
        with closing(sqlite3.connect(str(self.sources['decisions.db']), isolation_level=None)) as db:
        ...
        self.assertEqual(json.loads(self.sources['business_filing.json'].read_text()), {'version':1,'businesses':[]})
