...
class PaidPacingTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for patcher in (patch.dict(os.environ, {'TAHOR_PAID_REQUEST_INTERVAL_SECONDS': '3'}),
                        patch.object(classify, '_paid_next_start', {}),
                        patch.object(classify, '_paid_pacing_lock', threading.Lock()),
    ...
    def test_threads_share_start_reservations_without_real_waiting(self):
        with ThreadPoolExecutor(max_workers=8) as executor:
            list(executor.map(lambda _: classify.wait_for_paid_request(
                self.URL, self.MODEL, self.PROVIDER), range(8)))
        self.assertEqual(self.sleeps, [3.0] * 7)
        self.assertEqual(self.now, 121.0)
        self.assertEqual(list(classify._paid_next_start.values()), [124.0])

    def test_idle_period_has_no_catchup_burst_and_other_models_are_unpaced(self):
        classify.wait_for_paid_request(self.URL, self.MODEL, self.PROVIDER)
        self.now += 30
        classify.wait_for_paid_request(self.URL, self.MODEL, self.PROVIDER)
        classify.wait_for_paid_request(self.URL, self.MODEL, self.PROVIDER)
        self.assertEqual(self.sleeps, [3.0])
        classify.wait_for_paid_request(self.URL, 'x-ai/grok-4.6', {})
        classify.wait_for_paid_request('http://localhost/v1/chat/completions', self.MODEL, {})
        self.assertEqual(self.sleeps, [3.0])
