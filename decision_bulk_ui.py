...
(() => {
 ...
 function prioritize(preserve=true){
  ...
  const sections=[...new Set(ordered.filter(c=>!c.dataset.queued).map(c=>hasRecommendation(c)?'AI recommendations':'Other pending decisions'))];
  ...
  list.style.overflowAnchor='none';list.querySelectorAll('[data-decision-section]').forEach(heading=>heading.remove());let section='';for(const c of ordered){const next=c.dataset.queued?'':(hasRecommendation(c)?'AI recommendations':'Other pending decisions');if(next&&next!==section){const heading=document.createElement('h2');heading.dataset.decisionSection='';heading.textContent=next;list.append(heading);section=next;}list.append(c);}
