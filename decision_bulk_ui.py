...
(() => {
 ...
 function setBucket(c,value){const field=c.querySelector('[data-bulk-bucket]');if(!field)return;field.value=value;const menu=c.querySelector('[data-bulk-folder]');const custom=c.querySelector('[data-bulk-custom-folder]');if(!menu||!custom)return;const known=[...menu.options].some(option=>option.value&&option.value===value);menu.value=known?value:'';custom.value=known?'':value;}
 ...
 function prioritize(preserve=true){
  ...
  const hasRecommendation=c=>Boolean(c.querySelector('[data-decision-recommendation]')?.textContent.trim()&&(c.querySelector('[data-decision-choice]:checked')?.value||c.dataset.suggestedAction==='defer'));
  const deferredRecommendation=c=>['unsorted','skip'].includes(c.querySelector('[data-decision-choice]:checked')?.value)||c.dataset.suggestedAction==='defer';
  const sectionFor=c=>c.dataset.queued?'':(hasRecommendation(c)?(c.dataset.suggestedAction==='defer'?'AI needs more information':(deferredRecommendation(c)?'AI recommends dismissal or skipping':'AI recommendations')):'Other pending decisions');
  const rank=c=>c.dataset.queued?5:(hasRecommendation(c)?(c.dataset.suggestedAction==='defer'?1:(deferredRecommendation(c)?2:0)):(c.querySelector('[data-decision-choice]:checked')?.value?3:4));
  const ordered=[...current].sort((a,b)=>rank(a)-rank(b));
  const sections=[...new Set(ordered.filter(c=>!c.dataset.queued).map(sectionFor))];
  if(ordered.every((c,i)=>c===current[i])&&JSON.stringify([...list.querySelectorAll('[data-decision-section]')].map(h=>h.textContent))===JSON.stringify(sections))return;
  ...
  list.style.overflowAnchor='none';list.querySelectorAll('[data-decision-section]').forEach(heading=>heading.remove());let section='';for(const c of ordered){const next=sectionFor(c);if(next&&next!==section){const heading=document.createElement('h2');heading.dataset.decisionSection='';heading.textContent=next;list.append(heading);section=next;}list.append(c);}
 ...
 for(const c of cards()){const stored=saved[c.dataset.decisionId];const old=stored?.source_revision===c.dataset.decisionRevision?stored:null;if(old?.attentionJob)c.dataset.attentionJob=old.attentionJob;if(old?.instructionCleared)c.dataset.instructionCleared='true';if(old?.guidanceSaved)c.dataset.guidanceSaved='true';if(old?.manual){delete c.dataset.suggestedAction;choose(c,old.action,true);if(old.bucket!==undefined)setBucket(c,old.bucket);if(c.querySelector('[data-bulk-vendor]')&&old.vendor_name!==undefined)c.querySelector('[data-bulk-vendor]').value=old.vendor_name;}}
 document.addEventListener('card-guidance-state',()=>remember(),{signal});
 document.addEventListener('change',event=>{const c=event.target.closest('[data-decision-id]');if(c&&event.target.matches('[data-decision-choice],[data-bulk-bucket],[data-bulk-folder],[data-bulk-custom-folder],[data-bulk-vendor]')){if(event.target.matches('[data-bulk-folder]'))setBucket(c,event.target.value);if(event.target.matches('[data-bulk-custom-folder]')){c.querySelector('[data-bulk-bucket]').value=event.target.value;c.querySelector('[data-bulk-folder]').value='';}if(event.isTrusted){delete c.dataset.instructionCleared;delete c.dataset.guidanceSaved;}c.dataset.manual='true';delete c.dataset.suggestedAction;remember();count();}},{signal});
 document.addEventListener('input',event=>{if(event.target.matches('[data-bulk-custom-folder]')){const c=event.target.closest('[data-decision-id]');if(c&&event.isTrusted){delete c.dataset.instructionCleared;delete c.dataset.guidanceSaved;}event.target.dispatchEvent(new Event('change',{bubbles:true}));}},{signal});
 bar.querySelector('[data-select-all]').addEventListener('click',()=>{const action=bar.querySelector('[data-bulk-choice]').value;for(const c of cards())choose(c,action,true);remember();count();},{signal});
...
def choices(row, revision, suggestion=None, buckets=()):
    ...
    options = ([('keep', 'Keep'), ('keep_brief', 'Keep briefly'), ('trash', 'Trash permanently'), ('skip', 'Skip for now')]
               if row['kind'] == 'message_review' else [('map', 'Use this filing folder'), ('unsorted', 'Dismiss filing request — no rule')])
    selected = (suggestion or {}).get('action', '')
    if selected == 'defer': selected = ''
    radios = ''.join(f'<label style="display:block"><input data-decision-choice type="radio" name="decision-choice-{row["id"]}" value="{value}"{" checked" if selected == value else ""}> {label}</label>' for value, label in [('', 'Decide later — keep pending')] + options)
    fields = ''
    if row['kind'] == 'vendor_mapping':
        ...
        known_buckets = list(dict.fromkeys(buckets))
        folder_options = ''.join(f'<option value="{escape(b)}"{" selected" if b == bucket else ""}>{escape(b)}</option>' for b in known_buckets)
        custom_bucket = '' if bucket in known_buckets else bucket
        fields = (f'<div class="fields" style="margin-top:.75rem"><input type="hidden" data-bulk-bucket value="{escape(bucket)}">'
                  f'<label>Folder <select data-bulk-folder><option value="">Choose a folder</option>{folder_options}</select></label>'
                  f'<label>Or use a new folder <input type="text" data-bulk-custom-folder value="{escape(custom_bucket)}" placeholder="e.g. Health and fitness"></label>'
                  f'<label>Sender or organization <input type="text" data-bulk-vendor value="{escape(vendor)}"></label></div>'
                  '<p class="hint">Decide later keeps this request pending and is not submitted with the batch. Dismiss filing request closes it without creating a rule; Tahor will not ask again for this same sender. You can add a rule later.</p>'
    ...
    reason = (('AI: More information needed. Keep this request pending. ' if suggestion.get('action') == 'defer' else 'AI suggestion: ') + suggestion.get('reason', '')) if suggestion else ''
