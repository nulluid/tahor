...
def _card(item, year):
    ...
    details += f'<form method="post" action="/expenses/{identifier}/exclude">{hidden}<button type="submit">Remove from Expenses</button><span class="hint"> Removes this entry from the page and totals. Its audit record and email remain available in exports.</span></form>'
...
def render(entries, totals, *, year=None, report=None, years=None, archive_status=None):
    ...
    body += f'<p><a href="/expenses.csv?year={year}">Download {year} CSV</a> · <a href="/expenses.zip?year={year}">Download {year} audit ZIP</a> · <a href="/expenses.zip?scope=all">Download all expense data</a></p>'
    body += '<details><summary>About downloads and backups</summary><p class="hint">ZIPs contain the ledger, comments, categories, audit records, and available original .eml files, named by month, category, vendor, and entry ID. The manifest lists any originals still awaiting archive. Automatic recovery backups include this data; configure an off-server destination using the recovery instructions.</p></details>'
    if archive_status:
    ...
    if not active:
        body += '<p>No active expenses for this year. Removed entries remain in the audit export.</p>'
    return body
