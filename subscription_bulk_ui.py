...
(() => {
 ...
 function remember(){const state={};for(const c of cards()){const value=c.querySelector('input:checked')?.value;if(value||c.dataset.manual==='true')state[c.dataset.subscriptionId]={action:value,manual:c.dataset.manual==='true',attentionJob:c.dataset.attentionJob||''};}try{sessionStorage.setItem(storageKey,JSON.stringify(state));}catch(_){}}
 ...
 for(const c of cards()){const old=saved[c.dataset.subscriptionId];if(old?.attentionJob)c.dataset.attentionJob=old.attentionJob;if(old?.manual)choose(c,old.action,true);}
